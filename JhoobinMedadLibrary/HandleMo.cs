        ...
        public async static Task<bool> ReceivedMessage(MessageObject message, vw_servicesServicesInfo service)
        {
            ...
            try
            {
                ...
                using (var entity = new SharedLibrary.Models.ServiceModel.SharedServiceEntities(service.ServiceCode))
                {
                    ...
                    //    await SharedLibrary.MessageSender.MapfaOTPRequest(typeof(JhoobinMedadEntities), singleCharge, message, serviceAdditionalInfo);
                    //    return;
                    //}
                    ...
                    if (isUserSendsSubscriptionKeyword == true || isUserWantsToUnsubscribe == true)
                    {
                        ...
                        if (isCampaignActive == (int)CampaignStatus.Active && (serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Activated || serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Renewal))
                        {
                            ...
                            var result = await SharedLibrary.UsefulWebApis.DanoopReferral("http://79.175.164.52/JhoobinMedad/sub.php", string.Format("code={0}&number={1}&parent_code={2}&kc={3}", subId, message.MobileNumber, parentId, sha));
                        ...
                        else if ((isCampaignActive == (int)CampaignStatus.Active || isCampaignActive == (int)CampaignStatus.Suspend) && serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Deactivated)
                        {
                            ...
                            if (sub != null && sub.SpecialUniqueId != null)
                            {
                                subId = sub.SpecialUniqueId;
                                var sha = SharedLibrary.Security.GetSha256Hash(subId + message.MobileNumber);
                                var result = await SharedLibrary.UsefulWebApis.DanoopReferral("http://79.175.164.52/JhoobinMedad/unsub.php", string.Format("code={0}&number={1}&kc={2}", subId, message.MobileNumber, sha));
                            }
                        }
