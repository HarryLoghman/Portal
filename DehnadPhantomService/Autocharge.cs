...
namespace DehnadPhantomService
{
    class Autocharge
    {
        ...
        public void InsertAutochargeMessagesToQueue()
        {
            ...
            //        var autochargeTimeTable = GetAutochargeTimeTable();
            //        var mobilesList = PhantomLibrary.ContentManager.GetMobilesList();
            //        if (mobilesList == null)
            //            return;
            //        foreach (var item in autochargeTimeTable)
            //        {
            ...
            //                {
            //                    var autochargeContent = PhantomLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
            //                    if (autochargeContent == null)
            //                        continue;
            //                    autochargeContent.Content = PhantomLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                    autochargeContent.Content = PhantomLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            //                    var imiChargeObject = PhantomLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
            ...
            //            {
            //                var autochargeContent = PhantomLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
            //                if (autochargeContent == null)
            //                    continue;
            //                autochargeContent.Content = PhantomLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                autochargeContent.Content = PhantomLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            ...
            //            PhantomLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
            //        }
            //    }
            //}
            //catch (Exception e)
