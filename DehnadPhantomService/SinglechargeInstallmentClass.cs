    ...
    public class SinglechargeInstallmentClass
    {
        ...
        private void InstallmentJob(int installmentCycleNumber)
        {
            try
            {
                ...
                string aggregatorName = Properties.Settings.Default.AggregatorName;
                var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("Phantom", aggregatorName);
                List<SinglechargeInstallment> installmentList;
            ...
            try
            {
                using (var entity = new PhantomEntities())
                {
                    ...
                    foreach (var installment in chunkedSingleChargeInstallment)
                    {
                        ...
                        var response = PhantomLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = PhantomLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
