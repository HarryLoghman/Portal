        ...
        public void InsertAutochargeMessagesToQueue()
        {
            try
            {
                ...
                using (var entity = new MobiligaEntities())
                {
                    ...
                    var autochargeTimeTable = GetAutochargeTimeTable();
                    var mobilesList = MobiligaLibrary.ContentManager.GetMobilesList();
                    if (mobilesList == null)
                        return;
                    foreach (var item in autochargeTimeTable)
                    {
                        ...
                        foreach(var mobile in mobilesList)
                        {
                            ...
                            {
                                var autochargeContent = MobiligaLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
                                if (autochargeContent == null)
                                    continue;
                                autochargeContent.Content = MobiligaLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
                                autochargeContent.Content = MobiligaLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
                                var imiChargeObject = MobiligaLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
                        ...
                        {
                            var autochargeContent = MobiligaLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
                            if (autochargeContent == null)
                                continue;
                            autochargeContent.Content = MobiligaLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
                            autochargeContent.Content = MobiligaLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
                        ...
                        MobiligaLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
                    }
                }
            }
            catch (Exception e)
