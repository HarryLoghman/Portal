...
namespace DehnadDarchinService
{
    public class SinglechargeInstallmentClass
    {
        ...
        private static int maxChargeLimit = 7000;
        public void ProcessInstallment(int installmentCycleNumber)
        {
            try
            {
                ...
                Type entityType = typeof(DarchinEntities);
                ...
                using (var entity = new DarchinEntities())
                {
                    ...
                    SharedLibrary.InstallmentHandler.SamssonTciInstallmentJob(entityType, maxChargeLimit, installmentCycleNumber, serviceCode, installmentList, installmentListCount, installmentListTakeSize, serviceAdditionalInfo, singleChargeType);
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (installmentCycleNumber == 1)
                {
                    using (var portalEntity = new SharedLibrary.Models.PortalEntities())
                    {
                        ...
                        var service = SharedLibrary.ServiceHandler.GetServiceFromServiceCode("Darchin");
            ...
            try
            {
                using (var entity = new DarchinEntities())
                {
                    ...
                    foreach (var installment in chunkedSingleChargeInstallment)
                    {
                        ...
                        var response = DarchinLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = DarchinLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
