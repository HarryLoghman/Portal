...
namespace BehAmooz500Library
{
    ...
    {
        public HandleMo():base("BehAmooz500")
        {
        ...
        public async static Task<bool> ReceivedMessageOld2(MessageObject message, Service service)
        {
            ...
            using (var entity = new BehAmooz500Entities())
            {
                ...
                if (isUserSendsSubscriptionKeyword == true || isUserWantsToUnsubscribe == true)
                {
                    ...
                    {
                        var oldService = SharedLibrary.ServiceHandler.GetServiceFromServiceCode("AvvalYad");
                    ...
                    if (isCampaignActive == (int)CampaignStatus.Active && (serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Activated || serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Renewal))
                    {
                        ...
                        var result = await SharedLibrary.UsefulWebApis.DanoopReferral("http://79.175.164.52/behamooz500/sub.php", string.Format("code={0}&number={1}&parent_code={2}&kc={3}", subId, message.MobileNumber, parentId, sha));
                    ...
                    else if ((isCampaignActive == (int)CampaignStatus.Active || isCampaignActive == (int)CampaignStatus.Suspend) && serviceStatusForSubscriberState == SharedLibrary.HandleSubscription.ServiceStatusForSubscriberState.Deactivated)
                    {
                        ...
                        if (sub != null && sub.SpecialUniqueId != null)
                        {
                            subId = sub.SpecialUniqueId;
                            var sha = SharedLibrary.Security.GetSha256Hash(subId + message.MobileNumber);
                            var result = await SharedLibrary.UsefulWebApis.DanoopReferral("http://79.175.164.52/behamooz500/unsub.php", string.Format("code={0}&number={1}&kc={2}", subId, message.MobileNumber, sha));
                        }
                    }
