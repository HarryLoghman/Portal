...
namespace BulkLibrary
{
    public class BulkSenderController
    {
        ...
        public int prp_contentId { get; set; }
        /// <summary>
        ...
        private void sb_fill(bool getRetryOnes)
        {
            ...
            if (!getRetryOnes)
            {
                if (this.prp_readSize == 0)
                    this.v_lstEventbase = v_sharedServiceEntity.EventbaseMessagesBuffers.Where(o => o.ContentId == this.prp_contentId && o.MessageType == (int)SharedLibrary.MessageHandler.MessageType.EventBase
        ...
        {
            DateTime retryTimeOut = DateTime.Now.AddSeconds(-1 * this.prp_retryIntervalInSeconds);
            return v_sharedServiceEntity.EventbaseMessagesBuffers.Where(o => o.ContentId == this.prp_contentId && o.MessageType == (int)SharedLibrary.MessageHandler.MessageType.EventBase
        ...
        private void sb_sendList()
        {
            for (int i = 0; i <= this.v_lstEventbase.Count - 1; i++)
            {
                #region checkMonitoringStatus
                int monitoringStatus = this.v_sharedServiceEntity.MessagesMonitorings.Where(o => o.ContentId == this.prp_contentId).Select(o => o.Status).FirstOrDefault();
                while (monitoringStatus == (int)SharedLibrary.MessageHandler.ProcessStatus.Paused)
                {
                    monitoringStatus = this.v_sharedServiceEntity.MessagesMonitorings.Where(o => o.ContentId == this.prp_contentId).Select(o => o.Status).FirstOrDefault();
                }
                if (monitoringStatus == (int)SharedLibrary.MessageHandler.ProcessStatus.Failed
                     || monitoringStatus == (int)SharedLibrary.MessageHandler.ProcessStatus.Finished)
                    return;
                #endregion
