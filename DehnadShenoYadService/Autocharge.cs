...
namespace DehnadShenoYadService
{
    class Autocharge
    {
        ...
        public void InsertAutochargeMessagesToQueue()
        {
            ...
            //        var autochargeTimeTable = GetAutochargeTimeTable();
            //        var mobilesList = ShenoYadLibrary.ContentManager.GetMobilesList();
            //        if (mobilesList == null)
            //            return;
            //        foreach (var item in autochargeTimeTable)
            //        {
            ...
            //                {
            //                    var autochargeContent = ShenoYadLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
            //                    if (autochargeContent == null)
            //                        continue;
            //                    autochargeContent.Content = ShenoYadLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                    autochargeContent.Content = ShenoYadLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            //                    var imiChargeObject = ShenoYadLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
            ...
            //            {
            //                var autochargeContent = ShenoYadLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
            //                if (autochargeContent == null)
            //                    continue;
            //                autochargeContent.Content = ShenoYadLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                autochargeContent.Content = ShenoYadLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            ...
            //            ShenoYadLibrary.MessageHandler.InsertBulkMessagesToQueue(messages);
            //            ShenoYadLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
            //        }
            //    }
            //}
            //catch (Exception e)
