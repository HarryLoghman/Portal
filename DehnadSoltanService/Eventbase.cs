                ...
                using (var portalEntity = new PortalEntities())
                {
                    ...
                    foreach (var item in bulkContent)
                    {
                        try
                        {
                            ...
                            var imiChargeObject = SoltanLibrary.MessageHandler.GetImiChargeObjectFromPrice(0, null);
                            var aggregatorName = Properties.Settings.Default.AggregatorName;
                            var aggregatorId = SharedLibrary.MessageHandler.GetAggregatorIdFromConfig(aggregatorName);
                            var rnd = new Random();
                            ...
                            SoltanLibrary.MessageHandler.CreateMonitoringItem(contentId, SharedLibrary.MessageHandler.MessageType.EventBase, finalMobileNumbers.Count(), null);
                        }
                        catch (Exception e)
                        {
