...
{
    public class SendingMessage : IDisposable
    {
        #region general
        ...
        protected SendingMessage() : this(false)
        {
        }
        protected SendingMessage(bool addErrorDescription)
        {
        ...
        internal virtual void sb_finishRequest(WebRequestParameter parameter, Exception ex)
        {
            ...
            {
                Program.logs.Error(parameter.prp_service.ServiceCode + " : Exception in SendingMessage: ", ex);
                ...
                try
                {
                    if (webException.Response != null)
                    {
                        ...
                        {
                            bool isSucceeded;
                            parameter.prp_result = ex.Message + ":" + this.fnc_parseResult(parameter.prp_service, rd.ReadToEnd(), out isSucceeded);
                        }
                ...
                {
                    Program.logs.Error(parameter.prp_service.ServiceCode + " : Exception in SendingMessage inner try: ", ex1);
                }
            }
            else
            {
                ...
                Program.logs.Error(parameter.prp_service.ServiceCode + " : Exception in SendingMessage2: ", ex);
            }
        ...
        internal virtual void sb_finishRequest(WebRequestParameter parameter, bool httpOK, string result)
        {
            ...
            bool isSucceeded;
            parameter.prp_result = this.fnc_parseResult(parameter.prp_service, result, out isSucceeded);
            if (isSucceeded)
            {
        ...
        internal virtual void sb_saveResponseToDB(WebRequestParameter parameter)
        {
            ...
            try
            {
                ...
                Program.logs.Info(parameter.prp_service.ServiceCode + " : " + cmd.CommandText);
                cmd.Connection.Open();
                cmd.ExecuteNonQuery();
            ...
            {
                Program.logs.Error(parameter.prp_service.ServiceCode + " : Exception in saving to " + tableName + ": ", ex1);
            ...
            , int? price, string chargeKey)
        {
            ...
            requestBody = this.fnc_createBodyString(service, messageType, mobileNumber, messageContent, dateTimeCorrelator
                , price, chargeKey);
            WebRequestParameterMessage parameter = new WebRequestParameterMessage(id, mobileNumber, maxTries, dateTimeCorrelator, messageContent
                , enum_webRequestParameterType.message, messageType, requestBody, service, Program.logs);
        ...
        internal virtual string fnc_createBodyString(SharedLibrary.Models.vw_servicesServicesInfo service,
        ...
        internal virtual string fnc_parseResult(SharedLibrary.Models.vw_servicesServicesInfo service, string result, out bool isSucceeded)
        {
            isSucceeded = false;
            return "";
        }
