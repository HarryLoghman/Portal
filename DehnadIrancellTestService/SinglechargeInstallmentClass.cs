    ...
    public class SinglechargeInstallmentClass
    {
        ...
        private void InstallmentJob(int installmentCycleNumber)
        {
            try
            {
                ...
                var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("IrancellTest", aggregatorName);
                ...
                if (installmentCycleNumber == 1)
                {
                    using (var portalEntity = new SharedLibrary.Models.PortalEntities())
                    {
                        ...
                        var service = SharedLibrary.ServiceHandler.GetServiceFromServiceCode("IrancellTest");
            ...
            try
            {
                using (var entity = new IrancellTestEntities())
                {
                    ...
                    foreach (var installment in chunkedSingleChargeInstallment)
                    {
                        ...
                        var response = IrancellTestLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = IrancellTestLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
