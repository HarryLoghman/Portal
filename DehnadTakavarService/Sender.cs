        ...
        public void SendHandler()
        {
            try
            {
                ...
                using (var entity = new TakavarEntities())
                {
                    ...
                    autochargeMessages = TakavarLibrary.MessageHandler.GetUnprocessedAutochargeMessages(entity, readSize);
                    eventbaseMessages = TakavarLibrary.MessageHandler.GetUnprocessedEventbaseMessages(entity, readSize);
                    onDemandMessages = TakavarLibrary.MessageHandler.GetUnprocessedOnDemandMessages(entity, readSize);
            ...
            for (int i = 0; i < take.Length; i++)
            {
                using (var entity = new TakavarEntities())
                {
                    ...
                    if (aggregatorName == "Hamrahvas")
                        TaskList.Add(TakavarLibrary.MessageHandler.SendMesssagesToHamrahvas(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "PardisImi")
                        TaskList.Add(TakavarLibrary.MessageHandler.SendMesssagesToPardisImi(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Telepromo")
                        TaskList.Add(TakavarLibrary.MessageHandler.SendMesssagesToTelepromo(entity, chunkedMessages, serviceAdditionalInfo));
                }
            }
