            ...
            try
            {
                ...
                if (installmentCycleNumber == 1)
                {
                    using (var portalEntity = new SharedLibrary.Models.PortalEntities())
                    {
                        ...
                        var service = SharedLibrary.ServiceHandler.GetServiceFromServiceCode("Tamly");
            ...
            try
            {
                using (var entity = new TamlyEntities())
                {
                    ...
                    foreach (var installment in chunkedSingleChargeInstallment)
                    {
                        ...
                        var response = TamlyLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = TamlyLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
