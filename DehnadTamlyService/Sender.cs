            ...
            for (int i = 0; i < take.Length; i++)
            {
                using (var entity = new TamlyEntities())
                {
                    ...
                    if (aggregatorName == "Hamrahvas")
                        TaskList.Add(TamlyLibrary.MessageHandler.SendMesssagesToHamrahvas(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "PardisImi")
                        TaskList.Add(TamlyLibrary.MessageHandler.SendMesssagesToPardisImi(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Telepromo")
                        TaskList.Add(TamlyLibrary.MessageHandler.SendMesssagesToTelepromo(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Hub")
                        TaskList.Add(TamlyLibrary.MessageHandler.SendMesssagesToHub(entity, chunkedMessages, serviceAdditionalInfo));
                }
            }
