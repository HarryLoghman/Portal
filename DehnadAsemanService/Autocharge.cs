...
namespace DehnadAsemanService
{
    class Autocharge
    {
        ...
        public void InsertAutochargeMessagesToQueue()
        {
            ...
            //        var autochargeTimeTable = GetAutochargeTimeTable();
            //        var mobilesList = AsemanLibrary.ContentManager.GetMobilesList();
            //        if (mobilesList == null)
            //            return;
            //        foreach (var item in autochargeTimeTable)
            //        {
            ...
            //                {
            //                    var autochargeContent = AsemanLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
            //                    if (autochargeContent == null)
            //                        continue;
            //                    autochargeContent.Content = AsemanLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                    autochargeContent.Content = AsemanLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            //                    var imiChargeObject = AsemanLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
            ...
            //            {
            //                var autochargeContent = AsemanLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
            //                if (autochargeContent == null)
            //                    continue;
            //                autochargeContent.Content = AsemanLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                autochargeContent.Content = AsemanLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            ...
            //            AsemanLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
            //        }
            //    }
            //}
            //catch (Exception e)
