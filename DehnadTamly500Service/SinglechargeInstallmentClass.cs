    ...
    public class SinglechargeInstallmentClass
    {
        ...
        private void InstallmentJob(int installmentCycleNumber)
        {
            try
            {
                ...
                var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("Tamly500", aggregatorName);
        ...
        public async Task ProcessInstallmentChunk(List<SinglechargeInstallment> chunkedSingleChargeInstallment, Dictionary<string, string> serviceAdditionalInfo, List<ImiChargeCode> chargeCodes, int taskId, int installmentCycleNumber)
        {
            ...
            try
            {
                using (var entity = new Tamly500Entities())
                {
                    ...
                    {
                        if ((DateTime.Now.Hour == 23 && DateTime.Now.Minute > 45) || (DateTime.Now.Hour == 0 && DateTime.Now.Minute < 01))
                            break;
                        ...
                        var response = Tamly500Library.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = Tamly500Library.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
