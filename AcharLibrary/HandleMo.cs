    ...
    public class HandleMo:SharedShortCodeServiceLibrary.HandleMo
    {
        ...
        public async static Task<bool> ReceivedMessageOld2(MessageObject message, Service service)
        {
            ...
            using (var entity = new AcharEntities())
            {
                ...
                else if (message.Content == "00" || message.Content.Length == 7 || message.Content.Length == 8 || message.Content.Length == 9 || message.Content.ToLower().Contains("def") || message.Content.ToLower().Contains("abc"))
                {
