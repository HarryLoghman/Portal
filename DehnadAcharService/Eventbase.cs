        ...
        public void InsertEventbaseMessagesToQueue()
        {
            try
            {
                using (var entity = new AcharEntities())
                {
                    ...
                    AcharLibrary.MessageHandler.AddEventbaseMessagesToQueue(eventbaseContent, aggregatorId);
                }
            }
            catch (Exception e)
            {
                ...
                using (var portalEntity = new PortalEntities())
                {
                    ...
                    foreach (var item in bulkContent)
                    {
                        try
                        {
                            ...
                            var imiChargeObject = AcharLibrary.MessageHandler.GetImiChargeObjectFromPrice(0, null);
                            var aggregatorName = SharedLibrary.ServiceHandler.GetAggregatorNameFromServiceCode(Properties.Settings.Default.ServiceCode); ;
                            var aggregatorId = SharedLibrary.MessageHandler.GetAggregatorIdFromConfig(aggregatorName);
                            var rnd = new Random();
                            ...
                            AcharLibrary.MessageHandler.CreateMonitoringItem(contentId, SharedLibrary.MessageHandler.MessageType.EventBase, mobileNumbers.Count(), null);
                        }
                        catch (Exception e)
                        {
