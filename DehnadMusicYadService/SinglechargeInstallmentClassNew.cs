...
namespace DehnadMusicYadService
{
    public class SinglechargeInstallmentClassNew
    {
            ...
            try
            {
                ...
                using (var entity = new MusicYadEntities())
                {
                    ...
                    for (int installmentInnerCycleNumber = 1; installmentInnerCycleNumber <= 1; installmentInnerCycleNumber++)
                    {
                        ...
                        {
                            installmentListNotOrdered = portal.Subscribers.Where(o => o.ServiceId != 10039 && o.ServiceId != 10028 && o.ServiceId != 10025 && o.ServiceId != 10036).OrderBy(o => o.MobileNumber).Take(10000).Select(o => o.MobileNumber).ToList();
                        }
        ...
        private static Dictionary<string, int> getSubscribersDueToTotalPriceYesterday(MusicYadEntities entity)
        {
            ...
            , Dictionary<string, string> serviceAdditionalInfo, int tps, int maxTaskCount)
        {
            ...
            try
            {
                ...
                try
                {
                    cnn = MusicYadLibrary.publicVariables.GetConnection();
                    cnn.Open();
                }
                catch (Exception e)
                {
...
, timeStamp, mobile, rialedPrice, referenceCode, charge, serviceAdditionalInfo["aggregatorServiceId"], spId);
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    lock (obj)
                    {
                        v_throttle.throttleRequests("musicyad", mobile, guidStr);
                    }
                    timeBeforeSendMTNClient = DateTime.Now;
                    logs.Info("musicyad;" + mobile + ";" + guidStr + ";" + timeBeforeSendMTNClient.Value.ToString("hh:mm:ss.fff"));
