        ...
        public int ProcessInstallment(int installmentCycleNumber, int tpsOperator, int tps, DateTime lastExecutionTime, bool forciblyExecute)
        {
            ...
            try
            {
                ...
                using (var entity = new SharedLibrary.Models.ServiceModel.SharedServiceEntities(service.ServiceCode))
                {
                    ...
                    ////{
                    ////    installmentListNotOrdered = portal.Subscribers.Where(o => o.ServiceId != 10039 && o.ServiceId != 10028 && o.ServiceId != 10025 && o.ServiceId != 10036 && o.DeactivationDate == null).GroupBy(o => o.MobileNumber).Select(o => new { MobileNumber = o.Key }).OrderBy(o => o.MobileNumber).Skip(10000).Take(10000).Select(o => o.MobileNumber).ToList();
                    ////}
                    ...
                    //Dictionary<string, int> singleCharge = getSubscribersChargesPrice(entity);
            ...
            , Dictionary<string, string> serviceAdditionalInfo, int tpsOperator, int tps)
        {
            ...
            try
            {
                ...
                while (position < rowCount)
                {
                    ...
                    int currentTps = tps;
                    if (v_tpsDedicatedChanged) { v_tpsDedicatedChanged = false; currentTps = v_throttleDedicated.getMaxTps(tpsOperator, tps); logs.Info(";tpsChanged;MusicYad;" + tps + ";" + tpsOperator + ";" + currentTps + ";"); }
