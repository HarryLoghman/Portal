...
namespace DehnadJabehAbzarService
{
    class Eventbase
    {
                ...
                using (var portalEntity = new PortalEntities())
                {
                    ...
                    foreach (var item in bulkContent)
                    {
                        try
                        {
                            ...
                            SharedShortCodeServiceLibrary.MessageHandler.CreateMonitoringItem(Properties.Settings.Default.ServiceCode
                                ,contentId, SharedLibrary.MessageHandler.MessageType.EventBase, finalMobileNumbers.Count(), null);
                        }
                        catch (Exception e)
                        {
        ...
        public static async Task ProcessSubscribersListChunk(List<string> chunkedMobileNumbersList, string eventbaseContent, SharedLibrary.Models.ServiceModel.ImiChargeCode imiChargeObject
            , long serviceId, long aggregatorId, int taskId, long contentId)
        {
