        ...
        public void SendHandler()
        {
            try
            {
                ...
                using (var entity = new JabehAbzarEntities())
                {
                    ...
                    autochargeMessages = JabehAbzarLibrary.MessageHandler.GetUnprocessedAutochargeMessages(entity, readSize);
                    eventbaseMessages = JabehAbzarLibrary.MessageHandler.GetUnprocessedEventbaseMessages(entity, readSize);
                    onDemandMessages = JabehAbzarLibrary.MessageHandler.GetUnprocessedOnDemandMessages(entity, readSize);
            ...
            for (int i = 0; i < take.Length; i++)
            {
                using (var entity = new JabehAbzarEntities())
                {
                    ...
                    if (aggregatorName == "Hamrahvas")
                        TaskList.Add(JabehAbzarLibrary.MessageHandler.SendMesssagesToHamrahvas(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "PardisImi")
                        TaskList.Add(JabehAbzarLibrary.MessageHandler.SendMesssagesToPardisImi(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Telepromo")
                        TaskList.Add(JabehAbzarLibrary.MessageHandler.SendMesssagesToTelepromo(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Hub")
                        TaskList.Add(JabehAbzarLibrary.MessageHandler.SendMesssagesToHub(entity, chunkedMessages, serviceAdditionalInfo));
                }
            }
