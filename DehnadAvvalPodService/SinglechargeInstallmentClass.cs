            ...
            try
            {
                ...
                if (installmentCycleNumber == 1)
                {
                    using (var portalEntity = new SharedLibrary.Models.PortalEntities())
                    {
                        ...
                        var service = SharedLibrary.ServiceHandler.GetServiceFromServiceCode("AvvalPod");
            ...
            try
            {
                using (var entity = new AvvalPodEntities())
                {
                    ...
                    foreach (var installment in chunkedSingleChargeInstallment)
                    {
                        ...
                        var response = AvvalPodLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        ...
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                if (true == true) // TEMPORARY!!!!!!!
                                {
                                    ...
                                    response = AvvalPodLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
