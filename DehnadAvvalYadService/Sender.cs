    ...
    class Sender
    {
        ...
        public void SendHandler()
        {
            try
            {
                ...
                var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("AvvalYad", aggregatorName);
                ...
                using (var entity = new AvvalYadEntities())
                {
                    ...
                    autochargeMessages = AvvalYadLibrary.MessageHandler.GetUnprocessedAutochargeMessages(entity, readSize);
                    eventbaseMessages = AvvalYadLibrary.MessageHandler.GetUnprocessedEventbaseMessages(entity, readSize);
                    onDemandMessages = AvvalYadLibrary.MessageHandler.GetUnprocessedOnDemandMessages(entity, readSize);
            ...
            for (int i = 0; i < take.Length; i++)
            {
                using (var entity = new AvvalYadEntities())
                {
                    ...
                    if (aggregatorName == "Hamrahvas")
                        TaskList.Add(AvvalYadLibrary.MessageHandler.SendMesssagesToHamrahvas(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "PardisImi")
                        TaskList.Add(AvvalYadLibrary.MessageHandler.SendMesssagesToPardisImi(entity, chunkedMessages, serviceAdditionalInfo));
                    else if (aggregatorName == "Telepromo")
                        TaskList.Add(AvvalYadLibrary.MessageHandler.SendMesssagesToTelepromo(entity, chunkedMessages, serviceAdditionalInfo));
                }
            }
