        ...
        public static void FakeInstallmentJob()
        {
            ...
            try
            {
                using (var entity = new DefendIranEntities())
                {
                    ...
                    var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("DefendIran", "Telepromo");
                    foreach (var installment in installmentList)
                    {
                        if (batchSaveCounter >= 500)
                        {
            ...
            try
            {
                ...
                using (var entity = new DefendIranEntities())
                {
                    ...
                    foreach (var installment in installmentList)
                    {
                        ...
                        var response = DefendIranLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                        if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                response = DefendIranLibrary.MessageHandler.SendSinglechargeMesssageToTelepromo(message, serviceAdditionalInfo, installment.Id).Result;
                                if (response.IsSucceeded == false && response.Description.Contains("Billing  Failed"))
                                {
