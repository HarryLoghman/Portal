            ...
            try
            {
                ...
                using (var entity = new BimeKarbalaEntities())
                {
                    ...
                    foreach (var installment in installmentList)
                    {
                        ...
                        var response = BimeKarbalaLibrary.MessageHandler.SendSinglechargeMesssageToPardisImi(message, installment.Id);
                        if (response.IsSucceeded == false && response.Description.Contains("Insufficient balance"))
                        {
                            if (message.Price == 400)
                            {
                                ...
                                response = BimeKarbalaLibrary.MessageHandler.SendSinglechargeMesssageToPardisImi(message, installment.Id);
                                if (response.IsSucceeded == false && response.Description.Contains("Insufficient balance"))
                                {
                            ...
                            else if (message.Price == 300)
                            {
                                ...
                                if (response.IsSucceeded == false && response.Description.Contains("Insufficient balance"))
                                {
                                    ...
                                    {
                                        SetMessagePrice(message, chargeCodes, 50);
                                        response = BimeKarbalaLibrary.MessageHandler.SendSinglechargeMesssageToPardisImi(message);
                                    }
                                }
                            }
