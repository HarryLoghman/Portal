...
namespace DehnadHoshangService
{
    public class SinglechargeInstallmentClass
    {
        ...
        private void DeactivateChargingUsersAfter30Days()
        {
            try
            {
                using (var entity = new SharedLibrary.Models.ServiceModel.SharedServiceEntities("Hoshang"))
                {
        ...
        private void InstallmentJob(int installmentCycleNumber)
        {
            try
            {
                ...
                var serviceAdditionalInfo = SharedLibrary.ServiceHandler.GetAdditionalServiceInfoForSendingMessage("Hoshang", aggregatorName);
