...
namespace DehnadHoshangService
{
    class Autocharge
    {
        ...
        public void InsertAutochargeMessagesToQueue()
        {
            ...
            //        var autochargeTimeTable = GetAutochargeTimeTable();
            //        var mobilesList = HoshangLibrary.ContentManager.GetMobilesList();
            //        if (mobilesList == null)
            //            return;
            //        foreach (var item in autochargeTimeTable)
            //        {
            ...
            //                {
            //                    var autochargeContent = HoshangLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
            //                    if (autochargeContent == null)
            //                        continue;
            //                    autochargeContent.Content = HoshangLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                    autochargeContent.Content = HoshangLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            //                    var imiChargeObject = HoshangLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
            ...
            //            {
            //                var autochargeContent = HoshangLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
            //                if (autochargeContent == null)
            //                    continue;
            //                autochargeContent.Content = HoshangLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                autochargeContent.Content = HoshangLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            ...
            //            HoshangLibrary.MessageHandler.InsertBulkMessagesToQueue(messages);
            //            HoshangLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
            //        }
            //    }
            //}
            //catch (Exception e)
