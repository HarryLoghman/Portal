...
namespace DehnadMenchBazService
{
    class Autocharge
    {
        ...
        public void InsertAutochargeMessagesToQueue()
        {
            ...
            //        var autochargeTimeTable = GetAutochargeTimeTable();
            //        var mobilesList = MenchBazLibrary.ContentManager.GetMobilesList();
            //        if (mobilesList == null)
            //            return;
            //        foreach (var item in autochargeTimeTable)
            //        {
            ...
            //                {
            //                    var autochargeContent = MenchBazLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobile.Id);
            //                    if (autochargeContent == null)
            //                        continue;
            //                    autochargeContent.Content = MenchBazLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                    autochargeContent.Content = MenchBazLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            //                    var imiChargeObject = MenchBazLibrary.MessageHandler.GetImiChargeObjectFromPrice(autochargeContent.Price, null);
            ...
            //            {
            //                var autochargeContent = MenchBazLibrary.ContentManager.PrepareLatestMobileContent(subscriber.Id, mobilesList.FirstOrDefault().Id);
            //                if (autochargeContent == null)
            //                    continue;
            //                autochargeContent.Content = MenchBazLibrary.MessageHandler.AddAutochargeHeaderAndFooter(autochargeContent.Content);
            //                autochargeContent.Content = MenchBazLibrary.MessageHandler.HandleSpecialStrings(autochargeContent.Content, autochargeContent.Point, subscriber.MobileNumber, serviceId.Value);
            ...
            //            MenchBazLibrary.MessageHandler.InsertBulkMessagesToQueue(messages);
            //            MenchBazLibrary.MessageHandler.CreateMonitoringItem(null, SharedLibrary.MessageHandler.MessageType.AutoCharge, messages.Count, tag);
            //        }
            //    }
            //}
            //catch (Exception e)
